...
module.exports = function (rows) {
  ...
  rows.forEach(function (row) {
    ...
    // if the organisation tag has not been added
    if (!organisation_tags[row.org_tag_id]) {
    ...
    // if this is a new challenge
    if (!challenges[chal_id]) {
      // add it to the challenges object, referenced by id
      challenges[chal_id] = {id: chal_id, title: row.challenge_title, description: row.challenges_description, tags: [] };
    ...
    // if this tag is new to this particular challenge
    if (!challenge_tags[chal_id][row.challenge_tag_id]) {
