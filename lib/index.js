...
function register(server, options, next) {
  ...
  var peopleGetBy = function(constraint, value, cb) {
    return query(queries.peopleGetBy(constraint, value), pool, function(err, res) {
      //eslint-disable-line
      if (err) {
        console.log(err);
  ...
  var peopleAddPassword = function(userId, password, cb) {
    return query(queries.peopleAddPassword(userId, password), pool, function(err, res) {
      //eslint-disable-line
      if (err) {
        return cb(err);
      }
  ...
  var updateLastLogin = function(id, timestamp, cb) {
    return query(queries.updateLastLogin(id, timestamp), pool, function(err, res) {
      //eslint-disable-line
      if (err) {
        return cb(err);
      }
  ...
  var orgsGetByTag = function(activeOnly, filter, cb) {
    return query(queries.orgsGetByTag(activeOnly, filter), pool, function(err, res) {
      //eslint-disable-line
      if (err) {
        return cb(err, null);
      }
  ...
  var orgsSearch = function(searchTerm, searchTags, searchLocations, cb) {
    return query(queries.orgSearch(searchTerm, searchTags, searchLocations), pool, function(
      err,
      res
    ) {
      if (err) {
  ...
  var getMatchingChallenges = function(person, cb) {
    return query(queries.getMatchingChallenges(person), pool, function(err, res) {
      //eslint-disable-line
      if (err) {
        return cb(err);
      }
