...
function register (server, options, next) {
  ...
  var getAllPeople = function (activeAndNonAdmin, cb) {
    return query(queries.getAllPeople(activeAndNonAdmin), pool, function (err, res) { //eslint-disable-line
      if (err) {
        return cb(err);
      }
  ...
  var orgsSearch = function (searchTerm, searchTags, cb) {
    return query(queries.orgSearch(searchTerm, searchTags), pool, function (err, res) {
      if (err) {
        return cb(err);
      }
