...
var initialiseTrigger = require ('./helpers/initialise-trigger.js');
var formatters = require ('./formatters/index.js');

function register (server, options, next) {
  var pool = options.pool;
  var getAllPeople = function (cb) {
    return query (queries.getAllPeople (), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }

      return cb (null, formatters.getAllPeople (res.rows));
    });
  };

  var peopleGetBy = function (constraint, value, cb) {
    return query (queries.peopleGetBy (constraint, value), pool, function (
      err,
      res
    ) {
      //eslint-disable-line
      ...
      return cb (null, formatters.peopleGetById (res.rows));
    });
  };

  var peopleAddPassword = function (userId, password, cb) {
    return query (queries.peopleAddPassword (userId, password), pool, function (
      err,
      res
    ) {
      //eslint-disable-line
  ...
  var peopleAdd = function (user, cb) {
    return query (queries.peopleAdd (user), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var peopleToggleActive = function (id, cb) {
    return query (queries.peopleToggleActive (id), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var peopleGetByOrgId = function (orgId, cb) {
    return query (queries.peopleGetByOrgId (orgId), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var updateLastLogin = function (id, timestamp, cb) {
    return query (queries.updateLastLogin (id, timestamp), pool, function (
      err,
      res
    ) {
      //eslint-disable-line
  ...
  var orgsGetByTag = function (activeOnly, filter, cb) {
    return query (queries.orgsGetByTag (activeOnly, filter), pool, function (
      err,
      res
    ) {
      //eslint-disable-line
      ...
      if (res.rows.length === 0) {
        // i.e. no orgs -OR- filter --> wrong tagId or no orgs data
  ...
  var orgsEdit = function (id, orgObj, cb) {
    return query (queries.orgsEdit (id, orgObj), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var orgsAdd = function (org, cb) {
    return query (queries.orgsAdd (org), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var orgsToggleActive = function (id, cb) {
    return query (queries.orgsToggleActive (id), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var orgsGetActive = function (cb) {
    return query (queries.orgsGetActive, pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var orgsSearch = function (searchTerm, searchTags, searchLocations, cb) {
    return query (
      queries.orgSearch (searchTerm, searchTags, searchLocations),
      pool,
      function (err, res) {
        if (err) {
          return cb (err);
  ...
  var getMatchingPeople = function (person, cb) {
    return query (queries.getMatchingPeople (person), pool, function (
      err,
      res
    ) {
      //eslint-disable-line
  ...
  var getMatchingChallenges = function (person, cb) {
    return query (queries.getMatchingChallenges (person), pool, function (
      err,
      res
    ) {
      //eslint-disable-line
  ...
  var getMatchingOrgs = function (person, cb) {
    return query (queries.getMatchingOrgs (person), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var peopleGetTags = function (id, cb) {
    return query (queries.peopleGetTags (id), pool, function (err, res) {
      if (err) {
        return cb (err);
      }
  ...
  var peopleSearch = function (searchTerm, tags, cb) {
    return query (queries.peopleSearch (searchTerm, tags), pool, function (
      err,
      res
    ) {
      if (err) {
  ...
  var orgsAddTags = function (itemId, tagIds, cb) {
    query (queries.orgsAddTags (itemId, tagIds), pool, function (
      queryErr,
      res
    ) {
  ...
  var peopleAddTags = function (itemId, tagIds, cb) {
    query (queries.peopleAddTags (itemId, tagIds), pool, function (
      queryErr,
      res
    ) {
      return cb (queryErr, res.rows);
  ...
  var orgsAddLocations = function (itemId, tagIds, cb) {
    query (queries.orgsAddLocations (itemId, tagIds), pool, function (
      queryErr,
      res
    ) {
      return cb (queryErr, res.rows);
  ...
  var deleteUser = function (id, cb) {
    return query (queries.deleteUser (id), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
      if (res.rowCount === 0) {
        return cb (
          Boom.notFound ('Cannot delete user with challenges or comments')
        );
      }
  ...
  var deleteOrg = function (id, cb) {
    return query (queries.deleteOrg (id), pool, function (err, res) {
      //eslint-disable-line
      if (err) {
        return cb (err);
      }
  ...
  var initTrigger = initialiseTrigger (options);
  ...
  query (initQuery, pool, function (initErr) {
    ...
    return query (initTrigger, pool, function (triggerErr) {
      if (triggerErr) {
        return next (triggerErr);
      }
      ...
      server.method ('pg.people.peopleAddTags', peopleAddTags);
      server.method ('pg.people.peopleGetTags', peopleGetTags);
      server.method ('pg.organisations.orgsAddLocations', orgsAddLocations);
      server.method ('pg.organisations.orgsGetAll', orgsGetAll);
      server.method (
        'pg.organisations.getUsedLocationsIds',
        getUsedLocationsIds
      );
      server.method ('pg.people.getMatchingPeople', getMatchingPeople);
      server.method ('pg.people.getMatchingChallenges', getMatchingChallenges);
      server.method ('pg.people.getMatchingOrgs', getMatchingOrgs);
