...
function register (server, options, next) {
  ...
  var orgsGetAvailable = function (cb) {
    return query(queries.orgsGetAvailable, pool, function (err, res) { //eslint-disable-line
      if (err) {
        return cb(err);
      }
  ...
  query(initQuery, pool, function (initErr) {
    ...
    server.method('pg.organisations.getAvailable', orgsGetAvailable);
