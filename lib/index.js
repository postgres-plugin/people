...
var validatePeopleUpdate = require('./helpers/validate-people-update.js');
...
function register (server, options, next) {
  ...
  var peopleEdit = function (userId, updatedProfile, cb) {
    ...
    validatePeopleUpdate(fields, updatedProfile, function (err) {
      if (err) {
        return cb(null, err);
      }

      return query(queries.peopleEdit(userId, fields, updatedProfile), pool, function (err, res) { //eslint-disable-line
        if (err) {
          return cb(err);
        }
        if (res.rowCount === 0) {
          return cb(null, Boom.notFound('The user specified does not exist'));
        }
